...
double external_potential(double val)
{
	...
	double V = pow(val,4) -2.5*val*val;
	return V;
}
...
double variationalWaveFunction(double v)
{
	//return 1.0;
	double w=0.0;
	w = exp(-pow(v-mu,2.)/(2.*sigma2)) + exp(-pow(v+mu,2.)/(2.*sigma2));
	return w;
	//return exp(-0.5*v*v);
}
...
double variationalWaveFunction_second(double v)
{
	...
	return (-1./(2.*sigma2))*(exp(-(pow(v-mu,2.))/2*sigma2) + exp(-(pow(v+mu,2.))/2*sigma2)) + (1./(2.*sigma2*sigma2))*(pow((v-mu),2)*exp(pow((v-mu),2)/(2.*sigma2)) + pow((v+mu),2)*exp(pow((v+mu),2)/(2.*sigma2)));
}
...
void finalizeHistogram()
{
	...
	ofstream out(nome+to_string(temperature)+estensione);
